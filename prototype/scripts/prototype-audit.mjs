...
if(adminRuntimeReady){
 ...
 for(const token of ['window.Festacol','utils.routeUrl(\'admin\'','rawPage === \'users\'','normalizedPage','history.replaceState','window.addEventListener(\'popstate\'','modal: kind','data-dropdown-toggle="admin-notification-menu"','id="mobile-nav-dialog"','role="dialog"','backdrop-blur-sm','authorizeRewrite','rewriteArchivedAt','Exam ID','exam-detail-qr','whatsapp-form','whatsAppGroupForClass','data-attempt-detail','Integrity / proctor log','data-exam-edit','data-reset-unfinished','question-correct-option','question-fill-answer',"modal==='exam-created'","menu: ['M5 7h14M5 12h14M5 17h14']","close: ['M6 18 17.94 6M18 18 6.06 6']","more: ['M6 12h.01m6 0h.01m5.99 0h.01']","external: ['M18 14v4.833",'data-admin-dashboard','data-admin-metric','data-operations-queue','data-open-qr-link','target="_blank"','rel="noopener noreferrer"','w-64','lg:pl-64','animate-admin-enter','animate-toast-in'])if(!admin.includes(token))throw new Error(`Task 6 Admin runtime missing ${token}`);
...
if(targetRuntimeReady){
 ...
 for(const token of ['--animate-admin-enter','--animate-admin-pop','--animate-toast-in','@keyframes admin-enter','@media (prefers-reduced-motion: reduce)'])if(!indexHtml.includes(token))throw new Error(`Canonical index shell missing shared Admin motion contract: ${token}`);
