(() => {
  ...
  const validateQuestion = (question, ids) => {
    ...
    if ((question.type === 'fill' || question.type === 'fill-multi') && (!Array.isArray(question.fillTemplate) || !question.fillTemplate.some((part) => part && part.blank))) throw new Error(`Question ${question.id} needs a response blank.`);
    ids.add(question.id);
  };
  ...
  const resolveUrl = () => {
    ...
    if (script && script.src) return new URL('../data/questions.json', script.src).href;
  ...
  const questionsForSession = (payload, session) => {
    ...
    const subjectOrder = session.mode === 'qualifier' ? [...new Set(candidates.map((question) => question.subjectCode))] : session.subjects;
