...
export function StaffProvisionDialog() {
  ...
  useEffect(() => { if (open) void getAdminFormOptionsAction().then((options) => { setSubjects(options.subjects.filter((subject) => subject.kind === "curriculum")); setOfferings(options.offerings); }).catch(() => setError("Staff scope options could not be loaded.")); }, [open]);
