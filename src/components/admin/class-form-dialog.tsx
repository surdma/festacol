        ...
        <FieldGroup>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field>
              ...
              <NativeSelect id="class-level" value={classLevel} onChange={(event) => setClassLevel(event.target.value)}>
                {["SS1", "SS2", "SS3"].map((level) => <NativeSelectOption key={level} value={level}>{level}</NativeSelectOption>)}
              </NativeSelect>
            </Field>
            <Field>
              <FieldLabel htmlFor="class-track">Track</FieldLabel>
              <NativeSelect id="class-track" value={track} onChange={(event) => setTrack(event.target.value as AcademicTrack)}>
            ...
            <Field>
              <FieldLabel htmlFor="class-arm">Arm</FieldLabel>
              <Input id="class-arm" value={arm} onChange={(event) => setArm(event.target.value.toUpperCase())} maxLength={4} />
            </Field>
            <Field>
              <FieldLabel htmlFor="class-capacity">Capacity</FieldLabel>
              <Input id="class-capacity" type="number" min={1} max={500} value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} />
              <FieldDescription>Used for occupancy and remaining-place calculations.</FieldDescription>
            </Field>
          </div>
          <Field>
            <FieldLabel htmlFor="class-room">Room / location</FieldLabel>
            <Input id="class-room" value={room} onChange={(event) => setRoom(event.target.value)} maxLength={50} placeholder="Science Wing · Room 4" />
          </Field>
        </FieldGroup>
