...
export default async function AdminQuestionsPage({ searchParams }: { searchParams: Promise<{ q?: string; subject?: string; type?: string; source?: string }> }) {
  ...
  return (
    <div>
      <AdminPageHeader eyebrow="Assessment content" title="Question Bank" description="Questions belong to canonical subjects; staff-authored questions retain their creator while seeded bank questions are system-owned." actions={<Button render={<Link href="/workspace/questions?modal=question-new" />} className={adminPrimaryButtonClass}><Plus data-icon="inline-start" />New question</Button>} />
