  ...
  return (
    <div data-admin-dashboard>
      ...
      <section className={`${card} mt-5 overflow-hidden`}>
        ...
        {recent.length ? <div className="divide-y divide-neutral-100">{recent.map((attempt) => <Link key={attempt.id} href={`/admin/reports?modal=attempt&attempt=${encodeURIComponent(attempt.id)}`} className="flex items-center gap-3 p-4 hover:bg-neutral-50"><span className="grid size-9 place-items-center rounded-xl bg-neutral-100 text-xs font-extrabold">{initials(attempt.context_snapshot.studentName)}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{attempt.context_snapshot.studentName || "Candidate"}</strong><span className="mt-1 block truncate text-xs text-neutral-500">{attempt.context_snapshot.sessionTitle} · {attempt.submitted_at ? `Submitted ${Math.round(Number(attempt.score ?? 0))}%` : "In progress"}</span></span><Badge tone={attempt.submitted_at ? "emerald" : "amber"}>{attempt.submitted_at ? "submitted" : "in progress"}</Badge></Link>)}</div> : <Empty title="No candidate activity" detail="Attempts will appear here when students start an examination." />}
      </section>
    </div>
  );
}
