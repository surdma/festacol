...
export default async function AdminSettingsPage() {
    ...
    return (
      <div>
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          <article className={`${adminSurfaceClass} p-5`}><span className="grid size-9 place-items-center rounded-xl bg-neutral-950 text-white"><BookOpenCheck className="size-4" /></span><p className="mt-4 text-[10px] font-bold uppercase tracking-[.14em] text-neutral-500">Current scope</p><strong className="mt-2 block text-sm text-neutral-950">{scope.subjectIds.length ? `${scope.subjectIds.length} subject qualification${scope.subjectIds.length === 1 ? "" : "s"}` : "No subjects selected"}</strong><p className="mt-2 text-xs leading-5 text-neutral-500">Subjects control which examinations, questions, attempts and reports are visible to you.</p></article>
  ...
  const { data } = await supabase.from("subjects").select("id,name,active").order("name");
  const subjects = (data ?? []) as { id: string; name: string; active: boolean }[];
