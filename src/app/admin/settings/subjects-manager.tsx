  ...
  return (
    <section className={`${adminSurfaceClass} overflow-hidden`}>
      <div className="flex flex-col gap-3 border-b border-neutral-200 p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button type="button" variant="outline" disabled={pending} className={adminSecondaryButtonClass} onClick={() => run(seedSubjectsAction)}>Seed WAEC catalog</Button>
      </div>
      ...
      <div className="grid lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,.75fr)]">
        ...
        <div className="p-5">
          ...
          <FieldGroup className="mt-4">
            ...
            <Field><FieldLabel>Streams</FieldLabel><div className="flex flex-wrap gap-2">{STREAMS.map((stream) => <Button key={stream} type="button" size="sm" variant={streams.includes(stream) ? "default" : "outline"} className="rounded-lg" onClick={() => setStreams(streams.includes(stream) ? streams.filter((value) => value !== stream) : [...streams, stream])}>{stream}</Button>)}</div></Field>
