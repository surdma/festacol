      ...
      <Card>
        <Empty className="border-0 py-12">
          <EmptyHeader>
            ...
            <EmptyDescription>
              {hasFilters ? "Change the current search or status filter." : "Add the first student to begin building the academic directory."}
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            {hasFilters ? (
              ...
              <Button size="sm" render={<Link href="/admin/students?modal=user-new&role=student" />}><Plus data-icon="inline-start" />Add student</Button>
            )}
          </EmptyContent>
        </Empty>
      </Card>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead><span className="sr-only">Open student</span></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map(({ user, className, attempts }) => (
              <TableRow key={user.id} className="group">
                <TableCell>
                  <Link href={`/admin/students?modal=student&student=${encodeURIComponent(user.id)}`} className="flex min-w-0 items-center gap-3 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-ring">
                    ...
                    <span className="min-w-0">
                      <strong className="block truncate text-sm font-semibold group-hover:underline group-hover:underline-offset-4">{user.full_name}</strong>
                      <span className="mt-0.5 block truncate font-mono text-[11px] font-normal text-muted-foreground">{user.id}</span>
                    </span>
                  </Link>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <Button size="icon-sm" variant="ghost" render={<Link href={`/admin/students?modal=student&student=${encodeURIComponent(user.id)}`} />} aria-label={`Open ${user.full_name}`}><MoreHorizontal /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        ...
        {rows.map(({ user, className, attempts }) => (
          <Link key={user.id} href={`/admin/students?modal=student&student=${encodeURIComponent(user.id)}`} className="flex min-h-20 items-center gap-3 p-4 outline-none transition-colors hover:bg-muted/45 focus-visible:bg-muted/45">
            ...
            <span className="min-w-0 flex-1">
              <strong className="block truncate text-sm font-semibold">{user.full_name}</strong>
              <span className="mt-1 block truncate text-xs text-muted-foreground">{className} · {attempts} attempt{attempts === 1 ? "" : "s"}</span>
              <span className="mt-1 block truncate font-mono text-[10px] text-muted-foreground">{user.id}</span>
            </span>
...
export default async function AdminStudentsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const visible = users.filter((user) => {
    const matchesQuery = !normalizedQuery || `${user.full_name} ${user.id}`.toLowerCase().includes(normalizedQuery);
    const matchesStatus = status === "all" || (status === "active" ? user.status === "active" : user.status !== "active");
  ...
  const rows: DirectoryRow[] = await Promise.all(visible.map(async (user) => {
    let hash = "";
    try {
      hash = await studentHashFor(user.first_name, user.last_name);
    } catch {
      hash = "";
    }
    return {
      user,
      className: user.class_id ? classNames.get(user.class_id) ?? "Unassigned" : "Unassigned",
      attempts: hash ? attemptCounts.get(hash) ?? 0 : 0,
    };
  }));
  ...
  return (
    <div className="flex flex-col gap-5">
      <AdminPageHeader
        eyebrow="Directory"
        title="Students"
        description="Student records connect directly to class membership, exam history and integrity activity."
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-medium text-muted-foreground">
        <span>{rows.length} shown · {users.length} total</span>
        <span>{activeCount} active · {classes.length} classes</span>
      </div>
