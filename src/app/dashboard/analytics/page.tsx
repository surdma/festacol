...
export default async function AnalyticsPage() {
  ...
  if (!ctx) redirect("/?next=/dashboard/analytics");
  ...
  const subjectName = new Map(
    ((subjects ?? []) as { id: string; name: string }[]).map((row) => [
      row.id,
      row.name,
    ]),
  );
  const questionSubject = new Map(
    questionRows.map((row) => [row.id, row.subject_id]),
  );
  ...
  const subjectStats = [...aggregate.entries()].map(([subjectId, value]) => ({
    ...
    <FadeUp className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-muted-foreground">
          {latest.context_snapshot.sessionTitle}
        </p>
      </div>
      <Stagger className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        ...
        <MetricCard
          label="Completion"
          value={`${latest.completion ?? 0}%`}
          icon={ClipboardList}
        />
        <MetricCard
          label="Pace"
          value={String(latest.pace_index ?? 0)}
          icon={Gauge}
        />
        <MetricCard
          label="Integrity"
          value={`${latest.integrity_score ?? 100}%`}
          icon={ShieldCheck}
        />
      </Stagger>
      <Card>
          ...
          {subjectStats.map((stat) => (
            <div key={stat.subject} className="flex flex-col gap-1">
              <p className="flex justify-between text-sm">
                ...
                <span className="tabular-nums">{stat.percent}%</span>
              </p>
