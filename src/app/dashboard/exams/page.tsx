...
export default async function ExamsPage() {
  ...
  if (!ctx) redirect("/?next=/dashboard/exams");
  ...
  return (
    <FadeUp className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-muted-foreground">Every attempt on your record.</p>
      </div>
      {attempts.length === 0 ? (
        <EmptyState
          title="No exam attempts yet"
          description="Open an exam from a link, QR card, or the Exam ID button."
        />
      ) : (
            ...
            <Card key={attempt.id}>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-base">
                  ...
                  {attempt.context_snapshot.className
                    ? ` · ${attempt.context_snapshot.className}`
                    : ""}
                </CardTitle>
                <StatusBadge tone={attempt.submitted_at ? "emerald" : "amber"}>
                  {attempt.submitted_at ? "Submitted" : "In progress"}
                </StatusBadge>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                {attempt.submitted_at
                  ? `Score ${attempt.score ?? "—"}% · Integrity ${attempt.integrity_score ?? "—"}%`
